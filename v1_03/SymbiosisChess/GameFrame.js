...
import { getChessboardAvatars } from "./ChessboardAvatars.js";
...
let getGameFrame = function ({ globalState, sessionState, soundOutput, musicOutput, displayProcessing }) {
    ...
    let chessboardAvatars = getChessboardAvatars({ gameState: sessionState.gameState, displayProcessing });
    let shouldRenderChessboardAvatars = false;
    let chessboardAvatars_xOffset = 0;
    let chessboardAvatars_yOffset = 0;
    ...
    let chessboardUIOffscreenCanvas = null;
    let gameFrameButtons = getGameFrameButtons({ globalState: globalState, gameState: sessionState.gameState, isLandscapeOrientation: displayProcessing.isLandscapeOrientation(), displayProcessing });
    ...
    let updateUI = function (displayProcessing) {
        ...
        if (displayProcessing.isLandscapeOrientation()) {
            shouldRenderChessboardAvatars = canvasWidth >= 1200;
            chessboardUI_xOffset = shouldRenderChessboardAvatars
                ? (104 + Math.floor((canvasWidth - 1000) / 2))
                : (54 + Math.floor((canvasWidth - 1000) / 2));
            ...
            chessboardAvatars_xOffset = chessboardUI_xOffset - 100;
            chessboardAvatars_yOffset = chessboardUI_yOffset;
        }
        else {
            shouldRenderChessboardAvatars = false;
            ...
            powerTextDisplay_xOffset = chessboardUI_xOffset + 2;
            powerTextDisplay_yOffset = chessboardUI_yOffset - 10;
            chessboardAvatars_xOffset = 0;
            chessboardAvatars_yOffset = 0;
        }
    };
    ...
    let getNextFrame = function ({ keyboardInput, mouseInput, previousKeyboardInput, previousMouseInput, displayProcessing, soundOutput, musicOutput, thisFrame, elapsedMicrosThisFrame }) {
        var _a, _b, _c;
        updateUI(displayProcessing);
        chessboardAvatars.processFrame({ gameState: sessionState.gameState, elapsedMicrosThisFrame });
        if (sessionState.gameState.isGameCompleted && gameCompletedPanel === null) {
            ...
            let chess = new Chess();
            for (let previousMove of sessionState.gameState.previousMoves)
                chess.move(previousMove.moveSan);
            if (chess.isCheckmate() && (sessionState.gameState.boardState.isPlayerWhite !== sessionState.gameState.boardState.isWhiteTurn)) {
                let newHighestUnlockedOpponent;
                switch (sessionState.highestUnlockedOpponent) {
                    case 0 /* Opponent.Opponent1 */:
                        newHighestUnlockedOpponent = sessionState.gameState.opponent === 0 /* Opponent.Opponent1 */
                            ? 1 /* Opponent.Opponent2 */
                            : sessionState.highestUnlockedOpponent;
                        break;
                    case 1 /* Opponent.Opponent2 */:
                        newHighestUnlockedOpponent = sessionState.gameState.opponent === 1 /* Opponent.Opponent2 */
                            ? 2 /* Opponent.Opponent3 */
                            : sessionState.highestUnlockedOpponent;
                        break;
                    case 2 /* Opponent.Opponent3 */:
                        newHighestUnlockedOpponent = sessionState.gameState.opponent === 2 /* Opponent.Opponent3 */
                            ? 3 /* Opponent.Opponent4 */
                            : sessionState.highestUnlockedOpponent;
                        break;
                    case 3 /* Opponent.Opponent4 */:
                        newHighestUnlockedOpponent = sessionState.gameState.opponent === 3 /* Opponent.Opponent4 */
                            ? 4 /* Opponent.Opponent5 */
                            : sessionState.highestUnlockedOpponent;
                        break;
                    case 4 /* Opponent.Opponent5 */:
                        newHighestUnlockedOpponent = sessionState.gameState.opponent === 4 /* Opponent.Opponent5 */
                            ? 5 /* Opponent.OpponentCustom */
                            : sessionState.highestUnlockedOpponent;
                        break;
                    case 5 /* Opponent.OpponentCustom */:
                        newHighestUnlockedOpponent = sessionState.highestUnlockedOpponent;
                        break;
                }
                sessionState.highestUnlockedOpponent = newHighestUnlockedOpponent;
        ...
        if (sessionState.gameState === gameFrameButtonsResult.newGameState) {
        ...
        if (moveHistoryDisplayResult.selectedBoardState !== null)
            ...
            chessboardUI.updateBoardState({ boardState: sessionState.gameState.boardState });
        if (chessboardUIOffscreenCanvas === null)
            chessboardUIOffscreenCanvas = displayProcessing.getOffscreenCanvasImage({ width: CHESSBOARD_UI_WIDTH_IN_PIXELS + 20, height: CHESSBOARD_UI_WIDTH_IN_PIXELS + 20 });
        chessboardUIOffscreenCanvas.clearCanvas();
        chessboardUI.render({
            drawRectangle: function (x, y, width, height, color, fill) {
                if (chessboardUIOffscreenCanvas !== null)
                    chessboardUIOffscreenCanvas.drawRectangle(x + 10, y + 10, width, height, color, fill);
            },
            drawText: function (x, y, text, font, fontSize, color) {
                if (chessboardUIOffscreenCanvas !== null)
                    chessboardUIOffscreenCanvas.drawText(x + 10, y + 10, text, font, fontSize, color);
            },
            drawImageRotatedClockwise: function (image, imageX, imageY, imageWidth, imageHeight, x, y, degreesScaled, scalingFactorScaled, additionalOptions) {
                if (chessboardUIOffscreenCanvas !== null)
                    chessboardUIOffscreenCanvas.drawImageRotatedClockwise(image, imageX, imageY, imageWidth, imageHeight, x + 10, y + 10, degreesScaled, scalingFactorScaled, additionalOptions);
            }
        });
        ...
        if (gameFrameButtonsResult.clickedTooltipType !== null) {
            return getTooltipFrame({
                tooltipType: gameFrameButtonsResult.clickedTooltipType,
                isStartOfGame: false,
                gameFrame: thisFrame,
                globalState,
                sessionState,
    ...
    let render = function (displayOutput) {
        ...
        if (shouldRenderChessboardAvatars)
            chessboardAvatars.render(getTranslatedDisplayOutput(displayOutput, chessboardAvatars_xOffset, chessboardAvatars_yOffset));
        if (chessboardUIOffscreenCanvas !== null)
            displayOutput.drawOffscreenCanvasImageRotatedClockwise(chessboardUIOffscreenCanvas, 0, 0, chessboardUIOffscreenCanvas.getWidth(), chessboardUIOffscreenCanvas.getHeight(), chessboardUI_xOffset - 10, chessboardUI_yOffset - 10, 0, 128, null);
        else
